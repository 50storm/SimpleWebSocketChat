...
class ChatDBServer extends WSServer.Server {
  ...
  constructor(port) {
    ...
    gp.on("message", (socket, message) => {
      ...
      //修正：DB保存してから返信
      this.db.getLastMessage((id, from, to, message) => {
        // ログの取り出し
        console.log("=====DEBUG========");
        console.log("id" + id);
    ...
    gp.on("private_message", (socket, fromToMessage) => {
      // プライベートチャット
    ...
    gp.on("check_name", (socket, name) => {
      ...
      if (this.names[name] === undefined) {
        ...
        this.db.getMessage((id, from, to, message) => {
          // ログの取り出し
          if (to === "NULL") {
            // 全員宛のメッセージ
