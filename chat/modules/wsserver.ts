...
module WSServer {
  export class Server {
    ...
    // クライアントからの接続を待つ
    public start(): void {
      // listenイベント用のハンドラを設定する
      if (!this.handler) {
        this.app.listen(this.port);
      } else {
        this.app.listen(this.port, () => {
          this.handler.handler(this.port);
        });
      }
    }
    ...
    // グループを追加する
    public addGroup(group: WSServer.Group): void {
  ...
  export class Group {
    ...
    constructor(server: WSServer.Server, name: string = "") {
      ...
      this.group.on("connection", (socket: Socket) => {
    ...
    private onConnect(socket: Socket): void {
      // connection時にイベントハンドラをsocketに設定
