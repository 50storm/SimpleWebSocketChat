...
(function (WSServer) {
    ...
    Server.prototype.start = function () {
      ...
      if (!this.handler) {
        ...
        this.app.listen(this.port, function () {
          _this.handler.handler(_this.port);
        });
      }
    };
    // サーバにイベントハンドラを設定する
  ...
  var Group = /** @class */ (function () {
    function Group(server, name) {
      ...
      if (!this.group) {
        // 同じグループ名がすでにある場合は null
        console.log("エラー: すでに同じグループ名があります(" + name + ")");
        return;
      }
      this.name = name;
      // connection時のイベントハンドラを設定
      this.group.on("connection", function (socket) {
...
var ChatDB = /** @class */ (function (_super) {
  ...
  ChatDB.prototype.saveMessage = function (from, message, to) {
    ...
    var sql =
      "INSERT INTO " +
      this.table +
      " ( from_name,to_name,message,time) VALUES "; // SQL文の組み立て
      ...
      if (_this.names[name] === undefined) {
        ...
        _this.db.getMessage(function (id, from, to, message) {
          ...
          if (to === name || from === name)
            // 自分宛、自分からのメッセージ
            socket.emit("log_private_message", {
              id: id,
              from: from,
              to: to,
              message: message,
